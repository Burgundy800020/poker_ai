...
class PlayerAgent(Agent):
    ...
    def __init__(self, stream: bool = True):
        ...
        self.num_simulations = [750,2500,2000]
    ...
    def act(self, observation, reward, terminated, truncated, info):
        ...
        elif (win_ratio < self.fold_thresh[observation["street"]] * ((observation["opp_bet"]-observation["my_bet"])/100)*(1+max(0,self.winnings/500))):
            return action_types.FOLD.value, 0, -1
        elif (win_ratio > self.raise_thresh[observation["street"]]*(1+max(0,self.winnings/500)) and observation["valid_actions"][action_types.RAISE.value]):
            temp = win_ratio - self.raise_thresh[observation["street"]]*(1+max(0,self.winnings/500))
            temp /= 2.5
