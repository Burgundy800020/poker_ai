...
def fast_monte_carlo(observation, num_simulations = 500):
    ...
    my_discarded_card = [observation["my_discarded_card"]] if observation["my_discarded_card"] != -1 else []

    # Calculate equity through Monte Carlo simulation
    shown_cards = my_cards + community_cards + opp_discarded_card + opp_drawn_card + my_discarded_card
...
def compute_true_ratio(observation):
    my_cards = [int(card) for card in observation["my_cards"]]
    community_cards = [card for card in observation["community_cards"] if card != -1]
    opp_discarded_card = [observation["opp_discarded_card"]] if observation["opp_discarded_card"] != -1 else []
    opp_drawn_card = [observation["opp_drawn_card"]] if observation["opp_drawn_card"] != -1 else []
    ...
    win_count = 0
    lose_count = 0
    if (len(opp_discarded_card)==0 and observation["street"] == 3):
        for i in non_shown_cards:
            for j in non_shown_cards:
                if (i == j): continue
                my_hand_rank, opp_hand_rank = evaluate_hand((my_cards, [i,j], community_cards))
                if (my_hand_rank < opp_hand_rank):
                    win_count += 1
                elif (opp_hand_rank < my_hand_rank):
                    lose_count += 1
    elif (len(opp_discarded_card)>0 and observation["street"] == 3):
        for i in non_shown_cards:
            my_hand_rank, opp_hand_rank = evaluate_hand((my_cards, [i]+opp_drawn_card, community_cards))
            if (my_hand_rank < opp_hand_rank):
                win_count += 1
            elif (opp_hand_rank < my_hand_rank):
                lose_count += 1
    else: #observation["street"] == 2 and opponent discarded card
        for i in non_shown_cards:
            for j in non_shown_cards:
                if (i == j): continue
                my_hand_rank, opp_hand_rank = evaluate_hand((my_cards, [i]+opp_drawn_card, community_cards + [j]))
                if (my_hand_rank < opp_hand_rank):
                    win_count += 1
                elif (opp_hand_rank < my_hand_rank):
                    lose_count += 1
...
class PlayerAgent(Agent):
    ...
    def __init__(self, stream: bool = True):
        ...
        self.won_hands = 0
        self.winnings = 0
        self.raise_thresh = [1.5,1.3,1.1,1.0]
        self.fold_thresh = [1.1,1.05,1.0,1.0]
        self.num_simulations = [1000,3000,3000]
        # self.num_simulations = [500]*3
        
        self.prev_op_bet = -1
        self.weighted_opp_prior = 1.0
        self.total_opp_raises = [0]*4
    ...
    def act(self, observation, reward, terminated, truncated, info):
        ...
        if (observation["opp_bet"] > observation["my_bet"]):
            self.total_opp_raises[observation["street"]] += (observation["opp_bet"] - observation["my_bet"])

        if (info["hand_number"] != self.hand_number): #new hand
            self.hand_number = info["hand_number"]
            self.prev_op_bet = observation["opp_bet"]
            self.weighted_opp_prior = 1.0
            self.street = 0
        
        if (observation["street"] != self.street):
            self.weighted_opp_prior = max(1.0,self.weighted_opp_prior*0.8)

        mean_opp_raise = self.total_opp_raises[observation["street"]]*4 / info["hand_number"] if info["hand_number"] >= 50 else 30
        if (mean_opp_raise == 0): mean_opp_raise = 30
        if (self.prev_op_bet != observation["opp_bet"]):
            self.weighted_opp_prior += observation["opp_bet"]/mean_opp_raise
        
        if (observation["opp_last_action"] == 'CHECK' and observation["opp_bet"] < 100):
            self.weighted_opp_prior = max(1.0, self.weighted_opp_prior*0.9)
        
        # if (self.winnings/3 > (1000-info["hand_number"]+1) / 2 + 1):
        #     return action_types.FOLD.value, 0, -1

        if (observation["street"] == 3 or observation["street"] == 2 and observation["opp_discarded_card"] != -1):
            win_ratio = compute_true_ratio(observation)
        else:
            win_ratio = fast_monte_carlo(observation,num_simulations=self.num_simulations[observation["street"]])

        if (win_ratio < 0.9 and observation["valid_actions"][action_types.DISCARD.value]):
            best_discard, new_win_ratio = discard_monte_carlo(observation)
            return action_types.DISCARD.value, 0, best_discard
        elif (win_ratio < self.fold_thresh[observation["street"]] * ((observation["opp_bet"]-observation["my_bet"])/mean_opp_raise)*(1+max(0,self.winnings/500))):
            return action_types.FOLD.value, 0, -1
        elif (win_ratio > self.raise_thresh[observation["street"]]*(1+max(0,self.winnings/500))*self.weighted_opp_prior and observation["valid_actions"][action_types.RAISE.value]):
            temp = win_ratio - self.raise_thresh[observation["street"]]*(1+max(0,self.winnings/500))*self.weighted_opp_prior
            temp /= 2
            raise_amt = int(observation["min_raise"]+(observation["max_raise"]-observation["min_raise"])*min(1,temp))
            if (observation["street"] == 0): return action_types.RAISE.value, observation["min_raise"], -1
            else: return action_types.RAISE.value, raise_amt, -1
